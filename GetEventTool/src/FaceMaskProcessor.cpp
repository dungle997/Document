...
void FaceMaskProcessor::process(const std::string& ipcamera, const std::string& username, const std::string& userpwd){
    ...
    int lastId=1;
    std::ifstream file1;
    file1.open("../Data_EventId/" + ipcamera + "/facemask.txt", std::ios::in);
    if(file1.is_open()){
        file1 >> lastId;
    ...
    file1.close(); 
    /**/
    std::string urlJson = "http://" + ipCam + ":4003/event/facemask?lastId="+ std::to_string(lastId) + "&count=500";
    std::string urlFrame = "http://"+ ipCam + ":4003/event/frame/facemask/";
    ...
    std::string path = "../Json_Event/"+ ipcamera + "/facemask.txt";
    ...
    try {
        ...
        if (message != "[]"){
            for (auto event : j) {
                ...
                // }
                auto fmask = event["fmask"];
                int eventId = event["eventId"];
                std::cout << "eventId = "<<eventId << std::endl;
                std::string time = event["time"];
                time.replace(time.find(" "), 1, "_");
                // int frame = event["frame"];
                std::cout << "--------Face MASK Event-------" << std::endl;
                FaceMaskEvent* newEvent = new FaceMaskEvent(monitorId, boxes, time, eventId, ipcamera, fmask);
        ...
        /**/
            std::ofstream file;
            file.open("../Data_EventId/" + ipcamera + "/facemask.txt");
                    if(file.is_open()){
