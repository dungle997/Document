...
ObjectEvent::ObjectEvent(int cameraId, const std::vector<std::string>& objectName, const std::vector<cv::Rect>& boxes, const std::string& time, int frame, std::string ipcamera) {
...
void ObjectEvent::update(const cv::Mat& image) {
    ...
    if (!image.empty()){
    std::string nameImage = "../Event/" +this->ipcamera +"/objectdetect/" + name + "_" + this->time + "_" + std::to_string(this->frame) + ".jpg";
