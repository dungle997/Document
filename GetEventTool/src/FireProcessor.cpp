...
void FireProcessor::process(const std::string& ipcamera, const std::string& username, const std::string& userpwd) {
    std::string ipCam = ipcamera;
    this->access_token = GetImageEngine::get_Token(ipcamera, username, userpwd);
    /**/
    int lastId =1;
    std::ifstream file1;
    file1.open("../Data_EventId/" + ipcamera + "/firedetect.txt", std::ios::in);
    if(file1.is_open()){
        file1 >> lastId;
    ...
    file1.close();
    /**/
    std::string urlJson = "http://" + ipCam + ":4003/event/firedetect?lastId="+ std::to_string(lastId) +"&count=300";
    std::string urlFrame = "http://"+ ipCam + ":4003/event/frame/firedetect/";
    ...
    std::string path = "../Json_Event/" + ipcamera +"/firedetect.txt";
    ...
    try {
        ...
        if (message != "[]"){
            for (auto event : j) {
                ...
                //  /**/
                // int condition = comparison_firedetect(_eventId, eventId);
                // std::cout << "condition = " << condition << std::endl;
                // /**/
                    std::cout << "--------Fire Event-------" << std::endl;
                    FireEvent* newEvent = new FireEvent(monitorId, boxes, time, eventId, ipcamera);
            ...
            /**/
            std::ofstream file;
            file.open("../Data_EventId/" + ipcamera + "/firedetect.txt");
                    if(file.is_open()){
