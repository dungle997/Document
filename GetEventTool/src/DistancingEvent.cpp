...
DistancingEvent::DistancingEvent(int monitorId, const std::vector<cv::Rect> &boxes, const std::vector<std::vector<float>> &distances, const std::string &time, int frame, std::string ipcamera)
{
...
void DistancingEvent::update(const cv::Mat &image) {
    ...
    if (!image.empty()){
    std::string nameImage = "../Event/" + this->ipcamera + "/socialdistancing/" + this->time + "_" + std::to_string(this->frame) + ".jpg";
    cv::imwrite(nameImage, this->image);
    }
}
