...
FaceDetectEvent::FaceDetectEvent(int cameraId, const std::vector<cv::Rect>& boxes, const std::string& time, int frame, std::string ipcamera) {
...
void FaceDetectEvent::update(const cv::Mat& image) {
    ...
    if (!image.empty()){
    std::string nameImage = "../Event/" + this->ipcamera + "/facedetect/" + this->time + "_" + std::to_string(this->frame) + ".jpg";
