...
void DistancingProcessor::process(const std::string& ipcamera, const std::string& username, const std::string& userpwd) {
    std::string ipCam = ipcamera;
    this->access_token = GetImageEngine::get_Token(ipcamera, username, userpwd);
    /**/
    int lastId = 1;
    std::ifstream file1;
    file1.open("../Data_EventId/" + ipcamera + "/distancing.txt", std::ios::in);
    if(file1.is_open()){
        file1 >> lastId;
    ...
    file1.close();
    /**/
    std::string urlJson = "http://" + ipCam + ":4003/event/socialdistancing?lastId="+ std::to_string(lastId) + "&count=300";
    std::string urlFrame = "http://"+ ipCam + ":4003/event/frame/socialdistancing/";
    ...
    std::string path = "../Json_Event/" + ipcamera + "/distancing.txt";
        ...
        if (message != "[]"){
            for (auto event : j) {
                ...
                // int frame = event["frame"];
                    ...
                    std::cout << "--------Distancing Event-------" << std::endl;    
                    DistancingEvent* newEvent = new DistancingEvent(monitorId, boxes, distances, time, eventId, ipcamera);
            ...
            /**/
            std::ofstream file;
            file.open("../Data_EventId/" + ipcamera + "/distancing.txt");
                if (file.is_open()){
                    file << j[j.size()-1]["eventId"]; 
                    std::cout << "tim = " << j[j.size()-1]["eventId"] << std::endl;
                }   
            file.close(); 
            /**/ 
        }
