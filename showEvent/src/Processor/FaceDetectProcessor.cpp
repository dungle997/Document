...
FaceDetectProcessor::FaceDetectProcessor(int cameraId, DisplayBoard* displayBoard, int index, EventManager* eventManager) {
...
void FaceDetectProcessor::process(Mat& frame) {
   ...
   int w = 640;
    ...
    FaceDetectEvent* lastEvent = NULL;
    
    int viewIndex = 0;
    for (int index = 0; index < this->eventManager->faceDetectEvents.size(); index++) {
        FaceDetectEvent* event = this->eventManager->faceDetectEvents[index];
