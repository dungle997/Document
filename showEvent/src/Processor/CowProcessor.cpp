...
CowProcessor::CowProcessor(int cameraId, DisplayBoard* displayBoard, int index, EventManager* eventManager) {
...
void CowProcessor::process(Mat& frame) {
    ...
    CowEvent* lastEvent = NULL;
    
    int viewIndex = 0;
    for (int index = 0; index < this->eventManager->cowEvents.size(); index++) {
        CowEvent* event = this->eventManager->cowEvents[index];
    ...
    if (config.language == "VN") {
        title = "ĐẾM BÒ: " + std::to_string(count);
    }
    else {
        title = "NUMBER OF COW: " + std::to_string(count);
    }
