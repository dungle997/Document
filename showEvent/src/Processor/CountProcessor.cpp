...
CountProcessor::CountProcessor(int cameraId, DisplayBoard* displayBoard, int index, EventManager* eventManager) {
...
void CountProcessor::process(Mat& frame) {
    ...
    CountEvent* lastEvent = NULL;
    
    int viewIndex = 0;
    for (int index = 0; index < this->eventManager->countEvents.size(); index++) {
        CountEvent* event = this->eventManager->countEvents[index];
    ...
    if (config.language == "VN") {
        title = "ĐẾM NGƯỜI: " + std::to_string(count);
    }
    else {
        title = "NUMBER OF PEOPLE: " + std::to_string(count);
    }
