...
AlarmEvent::AlarmEvent(int cameraId, const std::vector<cv::Rect>& boxes, const std::string& time, std::string frame) {
...
void AlarmEvent::update(const cv::Mat& image) {
    this->image = image;
    ...
    for (auto box: this->boxes) {
        ...
        // std::string nameImage = "../personcount/" + this->time + "_" + std::to_string(this->frame) + ".jpg";
